<template>
  <div class="pg-container">
    <section class="banner1 flex items-center">
      <li class="title-banner wow animate__animated animate__fadeInUp">助力企业实现数智化升级引擎不熄</li>
    </section>
    <section class="section-wrapper text-center">
      ...
      <div class="flex justify-between flex-wrap space-y-1">
        <div @click="toCase(item.key)" class="card cursor-pointer flex justify-center items-center"
          v-for="(item, index) in cardConf" :key="index" :style="{ 'background-image': 'url(' + item.img + ')' }">
    ...
    <section class="section-wrapper affix-container text-center">
      ...
      <div class="menus flex justify-start flex-wrap">
        <span class="menu-tab cursor-pointer hvr-underline-from-center wow animate__animated animate__fadeInUp"
          v-for="(item, index) in cardConf" :key="index" @click="scrollTo(item.key)">
          {{ item.name }}
        </span>
      </div>
      <div class="case-caontainer">
        <div class="case-box" :id="items.key" v-for="(items, index) in cardConf" :key="index">
          <li class="name wow animate__animated animate__fadeInUp">{{ items.name }}</li>
          <el-carousel v-if="items.case[0].length" :interval="3000" :autoplay="false" class="carousel-imgs"
            height="302px">
            <el-carousel-item v-for="(item, index) in items.case" :key="index">
              <div class="case-list flex">
                <div class="case-item mx-2" v-for="(t, index) in item" :key="index">
          ...
          <div v-else class="case-list flex justify-start overflow-x-scroll">
            <div class="case-item flex flex-col hover:shadow-lg" v-for="(item, index) in items.case" :key="index">
      ...
      <li class="text-center wow animate__animated animate__fadeInUp">更多案例正在更新中…</li>
    </section>
  </div>
</template>
